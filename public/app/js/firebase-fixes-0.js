...
// ════════════════════════════════════════════════════════════════
// CRITICAL: Early goTo stub OUTSIDE any IIFE - true global
// Prevents "goTo is not defined" errors from inline onclick handlers
// when buttons are clicked before homework-1.js loads
// ════════════════════════════════════════════════════════════════
if (typeof window.goTo !== 'function') {
  window.__pendingGoTo = [];
  window.goTo = function GOTO_STUB(id) {
    console.log('[GOTO-STUB] Queuing navigation to', id);
    window.__pendingGoTo.push(id);
    
    var tryExec = function() {
      // Check if the real goTo has loaded (replaced our stub)
      if (typeof window.goTo === 'function' && window.goTo.name !== 'GOTO_STUB') {
        var pending = window.__pendingGoTo.slice();
        window.__pendingGoTo = [];
        pending.forEach(function(pid) { 
          try { window.goTo(pid); } catch(e) { console.error('goTo error:', e); }
        });
      } else {
...
/* ════════════════════════════════════════════════════════════════
   ...
   ────────────────────────────────────────────────────────────────
   Prevents users from accessing protected pages before Firebase
   auth has fully initialized.
   ════════════════════════════════════════════════════════════════ */

(function(){
  'use strict';
  
  // Wait for LTAuth to be available
  function waitForAuth(callback) {
    if (typeof window.LTAuth !== 'undefined' && window.LTAuth.ready) {
      callback();
    } else {
  ...
  // Initialize auth state watcher
  waitForAuth(function(){
    console.log('[AUTH] Firebase ready, watching auth state');
  ...
  function updateAuthUI(user) {
    ...
    var authRequired = document.querySelectorAll('[data-auth-required]');
    authRequired.forEach(function(el){
      if (user) {
        el.style.display = '';
      } else {
    ...
    if (currentPage && !user) {
      var protectedPages = ['pg-classroom', 'pg-homework', 'pg-profile', 'pg-parent', 'pg-arena', 'pg-exam'];
      var pageId = currentPage.id;
      if (protectedPages.includes(pageId)) {
        console.log('[AUTH] User logged out, redirecting from', pageId);
