...
function goTo(id){
  ...
  const publicPages = ['pg-landing', 'pg-languages', 'pg-beta'];
  
  // If going to a page that's NOT public, require auth
  if (!publicPages.includes(id) && id.startsWith('pg-')) {
    // Wait for Firebase auth to initialize
    if (typeof window.LTAuth === 'undefined' || !window.LTAuth.ready) {
      console.log('[AUTH] Firebase not ready, waiting...');
      setTimeout(function(){ goTo(id); }, 100);
      return;
    }
    
    // Check if user is logged in
    if (!window.LTAuth.user) {
      console.log('[AUTH] Not logged in, showing signup prompt');
      // Show a friendly message
      alert('Please sign up or log in to access this feature.\n\nThis allows:\n• Parents to monitor their children\n• Students to save progress\n• Family linking and progress tracking');
      goTo('pg-landing');
      return;
    }
  }
  
  // CRITICAL FIX: Force hide ALL pages first
...
// CRITICAL: Make goTo globally accessible immediately
window.goTo = goTo;
